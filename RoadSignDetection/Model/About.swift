...
struct Section {
    ...
    init(header: String, cells: [String]) {
        self.header = header
        self.cells = cells
    }
}
    ...
    static func getSections() -> [Section] {
        let sections = [
            ...
            Section(header: "Goal of the work",
                    cells: ["The purpose of this work is to create an ML model and develop an iOS application for search and recognition of road signs by photo."]),
            Section(header: "Initial data",
                    cells: ["The data for training the model was collected manually using search results and own photos taken outside in different weather conditions and under different lighting conditions."]),
            ...
            Section(header: "Model Training",
                    cells: ["""
                            ...
                            The advantages of this approach can be noted: running the model locally on the user's device eliminates the need for a network connection, which helps to maintain the confidentiality of user data and improves the response of the application.
