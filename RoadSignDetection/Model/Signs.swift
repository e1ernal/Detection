    ...
    static func getSigns() -> [Sign] {
        let signs = [
            ...
            Sign(name: "Priority road",
                 definition: "A road which is sign-posted as the priority road and on which moving vehicles are given way by those approaching from other directions when passing intersections",
                 imageURL: "main_road.png"),
            Sign(name: "Entry is prohibited",
                 definition: "Restricts entry to a certain part of the roadway for all vehicles, with the exception of shuttle buses and taxis",
            ...
            Sign(name: "Parking",
                 definition: "A sign, within the public right-of-way or adjacent thereto, that directs motorists to parking facilities",
                 imageURL: "parking.png")
        ]
        return signs.sorted { s1, s2 in
            s1.name < s2.name
        }
    }
}
